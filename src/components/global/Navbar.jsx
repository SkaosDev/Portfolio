...
function useAdaptiveColors(refs, enabled = true) {
    ...
    const sample = useCallback(() => {
        ...
        const newColors = refs.map((ref) => {
            if (!ref.current) return "black";
            ...
            for (const el of elements) {
                if (el.closest("header") || el.closest(".gradient-blur")) continue;
                ...
                if (bg && bg !== "rgba(0, 0, 0, 0)" && bg !== "transparent") {
                    ...
                    if (match) {
                        const lum = relativeLuminance(+match[1], +match[2], +match[3]);
        ...
        const key = newColors.join(",");
        if (key !== lastColorsRef.current) {
            lastColorsRef.current = key;
            setColors([...newColors]);
        }
    }, [refs, enabled]);
...
export default function Navbar({ onClickRef }) {
    ...
    const linkRefs = useRef(NAV_LINKS.map(() => ({ current: null })));

    const adaptiveColors = useAdaptiveColors(linkRefs.current, !open);
